    ...
    with(CoroutineScope(EmptyCoroutineContext)) {
        launch {
            try {
                ...
                val postsWithAuthor = getPosts(client)
                    .map { post ->
                        async {
                            PostWithAuthor(
                                post.id,
                                getAuthor(client, post.authorId).name,
                                post.content,
                                post.published,
                                post.likedByMe,
                                post.likes,
                ...
                val postsWithComments = postsWithAuthor.map { post ->
                    async {
                        val comments = getComments(client, post.id)
                        val commentsWithAuthors = comments
                            .map { comment ->
                                async {
                                    CommentWithAuthor(
                                        comment.id,
                                        comment.postId,
                                        getAuthor(client, comment.authorId).name,
                                        comment.content,
                                        comment.published,
                                        comment.likedByMe,
                                        comment.likes
                        ...
                        PostWithCommentsWithAuthors(post, commentsWithAuthors)
                    }
                }.awaitAll()
